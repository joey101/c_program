...
int array_second_positive(int size, int array[size]) {
    ...
    int current_positive = NOT_POSITIVE_ENOUGH;
    int current_value;
    int i = 0;
    while (i < size) {
        if (array[i] > 0) {
...
int main(void) {
    ...
    int size4 = 3;
    int array4[MAX_SIZE] = {1, -1, -1};

    int result4 = array_second_positive(size4, array4);
    if (result4 == NOT_POSITIVE_ENOUGH) {
        printf("array4 wasn't positive enough!\n");
    } 
    else {
        printf("The second positive value from array4 is: %d\n", result3);
    }
